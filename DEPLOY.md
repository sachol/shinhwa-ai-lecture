...
`/` 접속 → 자동으로 `/lecture/index.html`로 리다이렉트되어 강의 페이지 표시.
`/practice` 접속 → 실습 자료 페이지.
...
- `vercel.json`의 rewrites가 제대로 적용됐는지 확인
- 직접 `/lecture/index.html`로 접속해보고 정상이면 rewrite 문제
- Vercel 프로젝트 → Settings → Functions → Deployment Logs에서 에러 확인
